...
from VampireDiceRoller.DiscordRunner import without_command_prefix, get_token
...
def roll_command_handler(update_obj: telegram.Update, _: telegram.ext.CallbackContext):
    ...
    normal_dices, hunger_dices, comment = tokenize(without_command_prefix(update_obj.message.text))
    roll_result = roll(normal_dices, hunger_dices)
    update_obj.message.reply_markdown_v2(stringify(roll_result,
                                                   list_prefix='\\-',
                                                   bold_delimiter='*',
                                                   italic_delimiter='_'),
                                         quote=True)
...
if __name__ == '__main__':
    updater = telegram.ext.Updater(get_token())
